...
const ObtenerEstudiante = (request, response) => { 
    connection.query('SELECT p.Per_Id id, p.Per_Identificacion cedula, p.Per_PNombre pNombre, p.Per_SNombre sNombre, '+
                           'p.Per_PApellido pApellido, p.Per_SApellido sApellido, '+
                            ...
                            'e.Est_Viaja viaja, e.Est_Poliza poliza, '+
                            'DATE_FORMAT(e.Est_FecVenPoliza, "%Y-%m-%d")  as vencePoliza, '+
                            's.Sec_Grado grado, c.Ade_Nombre adecuacion, a.AEst_Descripcion descripcion '+
                        ...
                        'e.Est_Id = m.Est_Id AND p.Per_Identificacion = ? AND '+
